...
function createSegmentList() {
	...
	for (var i=0; i<SEGMENTCOUNT; i++) {
		if (gi > 0) {
			var p1 = V3.fromPolar(0, i*df, r);
			var p2 = V3.fromPolar(0, i*df+df, r);
			var s = p1.x < p2.x ? {a:p1, b:p2} : {a:p2, b:p1};
			s.d = s.b.sub(s.a);
			segmentList_.push(s);
			gi--;
...
MyActor.prototype.checkVectors = function(dp, dpOut) {
	var u = {a:this.position, b:this.position.add(dp)};
	u.d = u.b.sub(u.a);
	for (var i=0; i<segmentList_.length; i++) {
		...
		if (p != null) {
			...
			this.velocity = dir.scale(this.velocity.length()*elasticity_);
...
MyActor.prototype.respawn = function(checkX, checkY) {
	if (checkX !== false || checkY !== false) {
		...
		this.velocity = V3.fromPolar(0, 2*Math.PI*Math.random(), 0.08 + 0.5*Math.random());
...
MyActor.prototype.checkBox = function(dp, dpOut) {
	var pos = this.position.add(dp);
...
function friction(force) {
	force.scale(0.15);
}

var rotation_ = Math.PI/50;
var delta_ = 0;
...
Dbg.prln(GE.ctx.canvas.width + '|' + GE.ctx.canvas.height);
	...
	for (var i=0; i<ACTORCOUNT; i++) {
		...
		a_[i].respawn();
		// add callbacks
		a_[i].forces.push(gravity);
		//a_[i].forces.push(attraction);
		a_[i].forces.push(friction);
		a_[i].constrains.push(MyActor.prototype.checkVectors);
		a_[i].constrains.push(MyActor.prototype.checkBox);
	}
