...
(function() {
    function Lens() {
		...
		this.images = [];
		this.cursor = [0, 0];
		this.interpolations = ["none", "linear", "sinusoid"];
		this.ratio = [1, 1];
		this.radius = 0;
	};
	extend(Demo, Lens);

    Lens.prototype.initialize = async function initialize() {
	...
	Lens.prototype.resize = function resize(e) {
	...
	Lens.prototype.update = function update(frame, dt) {
	...
	Lens.prototype.render = function render(frame, dt) {
		glui.frontBuffer.blit(this.frontBuffer);
		//glui.frontBuffer.update(true);
	};
	Lens.prototype.onchange = function onchange(e, setting) {
