    ...
    (function() {
        ...
        Object.defineProperties(window, {
            'subtype': {
                writeable: false,
                enumerable: false,
                value: function(subType, superType) {
            ...
            'Module': {
                ...
                value: (function() {
                    ...
                    Module.prototype.resolveIncludes = async function() {
                        ...
                        var loads = [];
                        var mdl = this;
                        //console.log(`includes for ${mdl.url}`);
                        ...
                        return Promise.all(loads).then(
                            includes => {
                                for (var i=0; i<includes.length; i++) {
                                    if (includes[i].error instanceof Error) console.log(includes[i].error);
                                    else mdl.includes.push(includes[i]);
                    ...
                    Module.prototype.buildDependencyOrder = function() {
                        function dfs(mdl, order) {
                            //if (mdl.status == Resource.COMPLETE) {
                                for (var i=0; i<mdl.includes.length; i++) {
                                    var dm = mdl.includes[i];
                                    if (dm.status != Module.RESOLVED && !dm.visited) {
                                        dm.visited = true;
                                        dfs(dm, order);
                                    }
                                }
        ...
        if (this.includes.length > 0) console.log(order.map(x=>'-' + x.toString()).join('\n'));
                        // add included modules to DOM
                        for (var i=0; i<order.length; i++) {
                            var dm = order[i];
                            if (dm.status == Resource.COMPLETE) {
                                if (dm.node == null) {
                                    ...
                                    console.log(`${mdl.toString()} added already`);
                                }
                            } else {
                                console.log(`waiting for ${mdl.toString()}`);
                            }
                        }
        ...
        Object.defineProperties(window, {
            ...
            'include': {
                ...
                value: async function(path) {
                                ...
                                function checkState(resolve, count) {
                                    ...
                                    if (count++ == 3) {
                                        ...
                                        if (mdl.status === Module.RESOLVED || mdl.status === Module.ALIAS) {
                                            ...
                                            setTimeout(checkState, 100, resolve, count);
                                        }
                                    }
                                }
                                // await the fullfilled Promise with the status of the module
                                //await new Promise(resolve => { checkState(resolve); });
                    ...
                    if (mdl.status != Resource.COMPLETE && mdl.status !== Module.RESOLVED && mdl.status !== Module.ALIAS) {
