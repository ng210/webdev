...
// public(obj, name); => public(obj, name, url);
    this.data = this.data.replace(/public\s*\(\s*([^,]+)\s*,\s*([^)]+)\s*\)\s*;/, `public($1, $2, '${this.url}');`);
    eval(this.data);
    // this.node = document.createElement('script');
...
var rootUrl = (function(){
    var address = self.location.href; //document.currentScript.src;
...
var Dbg = {
    prln: txt => self.postMessage({code:'dbg', id:13, text:txt})
};
...
self.onmessage = async function(e) {
    ...
    switch (msg) {
        case 'initialize':
            if (data.root) {
                rootUrl = new Url(data.root);
            }
            if (data.path) {
                Resource.searchPath.push(...data.path);
            }
            var res = await load(data.module);
            if (res.error instanceof Error) {
                console.log('Sending kill signal');
                self.postMessage({code:'kill', id:e.data.id, data:res.error.message});
            } else {
                if (typeof res.symbols.main !== 'function') {
                    self.postMessage({code:'kill', id:e.data.id, data:`Entry point static function 'main' missing!`});
                }
                main();
            }
            break;
