...
class WebGL {
	...
	calculateTextureSize(count, forcePOT = true) {
		if (!forcePOT) {
	...
	deleteTexture(texture) {
		let ix = this.#textures.indexOf(texture);
		if (ix != -1) {
	...
	createFramebuffer(tag, texture) {
		let buffer = this.createBuffer(this.gl.FRAMEBUFFER, tag);
		buffer.bind(texture);
		return buffer;
	}

	deleteBuffer(buffer) {
		let ix = this.#buffers.indexOf(buffer);
		if (ix != -1) {
	...
	glCheck(label) {
  		...
  		if (err !== this.gl.NO_ERROR) {
    		console.error(`❌ WebGL error after ${label}: 0x${err.toString(16)}`);
    		debugger; // optional: stops at the error line
		}
  	}
}
